...
def test_pause_acquisition(tcp_server):
    ...
    assert tcp_server.response_commands == [{'msg': 'PAUSE', 'args': [1], 'kwargs': {}}]


def test_delete_all_grids(tcp_server):
    tcp_server.delete_all_grids()
    assert tcp_server.response_commands == [{'msg': 'DELETE ALL ARRAY GRIDS', 'args': [], 'kwargs': {}}]
...
def test_activate_grid(tcp_server):
    ...
    assert tcp_server.response_commands == [{'msg': 'ACTIVATE ARRAY GRID', 'args': [1], 'kwargs': {}}]


def test_deactivate_grid(tcp_server):
    tcp_server.deactivate_grid(roi_id=1)
    assert tcp_server.response_commands == [{'msg': 'DEACTIVATE ARRAY GRID', 'args': [1], 'kwargs': {}}]


def test_activate_overview(tcp_server):
    tcp_server.activate_overview(ov_id=1)
    assert tcp_server.response_commands == [{'msg': 'ACTIVATE OV', 'args': [1], 'kwargs': {}}]
...
def test_server_receives_and_sends_data(tcp_server, mocker):
    ...
    mock_conn.recv.return_value = json.dumps({"command": "TEST"}).encode("utf-8")
    ...
    mock_conn.sendall.assert_called_once_with(json.dumps({'commands': [{'msg': 'PAUSE', 'args': [1], 'kwargs': {}}]}).encode("utf-8"))
