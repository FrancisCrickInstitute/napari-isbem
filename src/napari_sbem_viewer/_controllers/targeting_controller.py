...
class TargetingController:
    ...
    def _connect_signals(self):
        ...
        self.view.add_labels.upload_labels_button.clicked.connect(
            self._on_click_upload_labels
        )
        self.view.label_settings.autofill_checkbox.stateChanged.connect(
            self._update_autofill_checkbox
        )
        self.view.label_settings.export_labels_button.clicked.connect(
            self._on_click_export_labels
        )
        self.view.label_settings.connected_components_button.clicked.connect(
            self.model.connected_components
        )
        self.view.label_settings.merge_nearby_labels_button.clicked.connect(
            self._on_click_merge_nearby_labels
        )
        self.view.label_settings.dilate_labels_button.clicked.connect(
            self._on_click_dilate_labels
        )
        self.view.label_settings.reset_labels_button.clicked.connect(
        ...
        # Model events
        self.model.interpolation_progress_updated.connect(
            self.view.label_settings.progress_bar.setValue
        )
        self.model.interpolation_started.connect(
            lambda: self.view.label_settings.interpolate_button.setEnabled(
                False
            )
        )
        self.model.interpolation_finished.connect(
            lambda: self.view.label_settings.interpolate_button.setEnabled(
                True
            )
        )
        self.model.autofill_labels = (
            self.view.label_settings.autofill_checkbox.isChecked()
        )
    ...
    def _on_click_add_labels(self):
        try:
            downsample_factor = (
                self.view.add_labels.downsample_combo_box.currentText()
            )
    ...
    def _on_click_merge_nearby_labels(self):
        self.model.merge_nearby_labels(
            self.view.label_settings.merge_tolerance_spinbox.value()
        )

    def _on_click_dilate_labels(self):
        self.model.dilate_labels(
            self.view.label_settings.dilate_amount_spinbox.value()
        )
