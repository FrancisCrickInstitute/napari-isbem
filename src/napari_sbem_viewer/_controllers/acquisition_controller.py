...
class AcquisitionController:
    def __init__(self, view, acquisition_model):
        ...
        self._on_change_pause_before_acquire_roi(
            self.view.acquisition_settings.pause_before_acquire_roi_checkbox.checkState()
        )
        self._on_change_pause_after_acquire_roi(
            self.view.acquisition_settings.pause_after_acquire_roi_checkbox.checkState()
        )

    def _init_signals(self):
        # Init acquisition model
        self.acquisition_model.acquisition_info_updated.connect(
            self.view.acquisition_info.update_acquisition_info
        )
        self.acquisition_model.rois_updated.connect(
            self.view.roi_settings.update_roi_info
        )
        self.acquisition_model.set_fine_thickness(
            self.view.acquisition_settings.fine_thickness_spinbox.value()
        )
        self.acquisition_model.errored.connect(self.view.show_error)
        self.acquisition_model.live_viewer.initialized.connect(
            self._on_add_overview
        )
        self.acquisition_model.live_viewer.cleared.connect(
            self._on_reset_overview
        )
        self.acquisition_model.live_viewer.errored.connect(
            self.view.show_error
        )

        # Init tcp settings
        self.view.tcp_settings.start_server_button.clicked.connect(
            self._on_click_start_server
        )
        self.view.tcp_settings.stop_server_button.clicked.connect(
            self._on_click_stop_server
        )

        # Init acquisition settings
        self.view.acquisition_settings.select_overview_dir_button.clicked.connect(
            self._on_change_ov
        )
        self.view.acquisition_settings.fine_thickness_spinbox.valueChanged.connect(
            self.acquisition_model.set_fine_thickness
        )
        self.view.acquisition_settings.pause_before_acquire_roi_checkbox.stateChanged.connect(
            self._on_change_pause_before_acquire_roi
        )
        self.view.acquisition_settings.pause_after_acquire_roi_checkbox.stateChanged.connect(
            self._on_change_pause_after_acquire_roi
        )

        # Init roi settings
        self.view.roi_settings.roi_combo_box.currentIndexChanged.connect(
            self._on_roi_layer_changed
        )
        ...
        self.view.acquisition_info.reset_view_button.clicked.connect(
            self._on_reset_view
        )

        # Init viewer events
        self.acquisition_model.viewer.layers.events.inserted.connect(
            self._on_add_layer
        )
        self.acquisition_model.viewer.layers.events.removed.connect(
            self._on_remove_layer
        )
        self.acquisition_model.viewer.dims.events.current_step.connect(
            self._on_change_z_depth
        )

    def _on_add_overview(self):
        self.view.roi_settings.reset()
        self.view.acquisition_settings.coarse_thickness_label.setText(
            f'{self.acquisition_model.live_viewer.pixel_size_z * 1e3:.0f}'
        )
        self.view.acquisition_settings.overview_dir_line.setText(
            self.acquisition_model.live_viewer.image_dir
        )
    ...
    def _on_change_z_depth(self):
        ...
        self.view.acquisition_info.viewer_z_depth.setText(
            f'{viewer_z_depth:.2f}µm'
        )
    ...
    def _on_remove_layer(self, event):
        if isinstance(event.value, Labels):
            idx = self.view.roi_settings.roi_combo_box.findText(
                event.value.name
            )
            if idx >= 0:
    ...
    def _on_click_start_server(self):
        self.acquisition_model.tcp_server.host = (
            self.view.tcp_settings.host_line_edit.text()
        )
        self.acquisition_model.tcp_server.port = (
            self.view.tcp_settings.port_spinbox.value()
        )
    ...
    def _get_roi_layer_names(self):
        return [
            x.name
            for x in self.acquisition_model.viewer.layers
    ...
    def _get_roi_layer(self):
        ...
        try:
            return self.acquisition_model.viewer.layers[
                self.view.roi_settings.roi_combo_box.currentText()
            ]
        except KeyError:
            return None
