...
from napari_isbem._models import LiveViewer, ROIData, TCPServer
from napari_isbem._models.acquisition_model import AcquisitionModel
...
def test_process_request_valid(acquisition_model, mocker):
    ...
    acquisition_model.live_viewer.is_initialized = mocker.MagicMock(
        return_value=True
    )
    ...
    acquisition_model.acquisition_info_updated.emit.assert_called_with(
        0.5, 0.1, True
    )
...
def test_process_request_error(acquisition_model, mocker):
    mock_request = {'slice_thickness': 50, 'z_depth': 100, 'paused': False}
    acquisition_model.tcp_server = mocker.MagicMock()
    acquisition_model.live_viewer.is_initialized = mocker.MagicMock(
        return_value=False
    )
    acquisition_model.process_request(mock_request)
    acquisition_model.errored.emit.assert_called_once_with(
        'LiveViewer error', 'Select overview directory before using TCP'
    )
...
def test_set_roi_layer_with_labels(acquisition_model, mocker):
    ...
    acquisition_model.roi_data.set_offset.assert_called_once_with(
        [
            acquisition_model.live_viewer.position_z,
            -acquisition_model.live_viewer.size_y // 2,
            -acquisition_model.live_viewer.size_x // 2,
        ]
    )
    acquisition_model.roi_data.add_masks.assert_called_once_with(roi_layer)
    acquisition_model.rois_updated.emit.assert_called_once_with(
        acquisition_model.roi_data
    )
...
@pytest.mark.parametrize(
    'pixel_size_z, fine_thickness, expected',
    [
        (0.1, 200, False),
        (0.1, 300, False),
        (0.4, 200, True),
...
def test_check_fine_thickness(
    acquisition_model, pixel_size_z, fine_thickness, expected
):
    ...
    if expected:
        ...
        with pytest.raises(
            ValueError,
            match='Fine thickness must be a multiple of coarse thickness.',
        ):
            acquisition_model._check_fine_thickness()
