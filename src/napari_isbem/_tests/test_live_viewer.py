...
from napari_isbem._models.live_viewer import LiveViewer


class MockTiff:
    def __init__(self, filename, metadata=None, shape=(10, 10)):
...
def mock_tiff_dir(tmp_path):
    def _mock_tiff_dir(mock_tiffs: list[MockTiff]):
        ...
        if not mock_tiff_dir.exists():
            mock_tiff_dir.mkdir()
            imwrite(
                'grab.tif', np.zeros((5, 5))
            )  # simulate grab file during SEM acquisition
        for mock_tiff in mock_tiffs:
            ...
            imwrite(
                mock_tiff_file, data, metadata=mock_tiff.metadata, ome=True
            )
...
def test_init_one_image(mock_tiff_dir, live_viewer):
    ...
    with pytest.raises(
        ValueError, match='Image directory must contain at least 2 images.'
    ):
...
def test_init_images(mock_tiff_dir, live_viewer):
    mock_tiffs = [
        MockTiff(
            'test_image_1.tif',
            shape=(10, 10),
            metadata={'Plane': {'PositionZ': 0.1}},
        ),
        MockTiff(
            'test_image_2.tif',
            shape=(10, 10),
            metadata={'Plane': {'PositionZ': 0.2}},
        ),
        MockTiff(
            'test_image_3.tif',
            shape=(10, 10),
            metadata={'Plane': {'PositionZ': 0.25}},
        ),
        MockTiff(
            'test_image_4.tif',
            shape=(10, 10),
            metadata={'Plane': {'PositionZ': 0.3}},
        ),
    ]
    mock_image_dir = mock_tiff_dir(mock_tiffs)
    live_viewer.init_images(mock_image_dir)
    assert live_viewer.layer.data.shape == (3, 10, 10)
    assert not live_viewer.watching
    ...
    assert live_viewer.position_x == 5000.0
    assert live_viewer.position_y == 10.0
    assert live_viewer.position_z == 0.1
    assert live_viewer.size_x == 1.0
    assert live_viewer.size_y == 2.0
    assert live_viewer.dtype == np.uint8
    assert live_viewer.layer is not None
    assert live_viewer.added_files == {
        'test_image_1.tif',
        'test_image_2.tif',
        'test_image_4.tif',
    }
    assert live_viewer.skipped_files == {'test_image_3.tif'}
...
def test_init_incorrect_z_spacing(mock_tiff_dir, live_viewer):
    ...
    with pytest.raises(
        ValueError,
        match='Inconsitent Z spacing between images. Expected: 0.20, Got: 0.30',
    ):
        live_viewer.init_images(mock_tiff_dir(mock_tiffs))
