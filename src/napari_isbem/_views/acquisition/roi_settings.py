...
from napari_isbem._models import ROIState
    ...
    def update_roi_info(self, roi_data):
        ...
        for r, roi in enumerate(roi_data.rois):
            for c, val in enumerate(
                [f'{roi.id}', f'{roi.z1:.2f}', f'{roi.z2:.2f}']
            ):
                ...
                if roi.state == ROIState.ACQUIRING:
                    self.model.item(r, c).setData(
                        QColor('green'), Qt.TextColorRole
                    )
                elif roi.state == ROIState.ACQUIRED:
                    self.model.item(r, c).setData(
                        QColor('grey'), Qt.TextColorRole
                    )
